...
import { ApiBody, ApiDefaultResponse, ApiQuery, ApiResponse, ApiTags } from '@nestjs/swagger';
...
export class MoviesController {
    ...
    @ApiResponse({ status: 200, description: 'Successful got all movies ' })
    @ApiResponse({ status: 400, description: 'Cannot get movies' })
    @Get()
    ...
    @ApiBody({ type: CreateMovieDto, description: 'Data for creating movie' })
    ...
    @ApiBody({ type: UpdateMovieDto, description: 'Data for creating movie' })
    @ApiResponse({ status: 201, description: 'Movie updated successfully' })
    @ApiResponse({ status: 400, description: 'Failed to update movie' })
    @Patch(":id")
