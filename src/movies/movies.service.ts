...
export class MoviesService {
	...
	async createMovie(movieData: CreateMovieDto): Promise<Movie> {
		try {
			...
			await this.mysqlConnection.query(
				'INSERT INTO movie (title, genre, language, duration) VALUES (?, ?, ?, ?);', [...Object.values(movieData),]
			);
	...
	async updateMovie(movieId: number, movieData: UpdateMovieDto): Promise<Movie> {
		try{
			...
			await this.mysqlConnection.query(
				'UPDATE movie SET title = ?, genre = ?, language = ?, duration = ? WHERE id = ?;', [...Object.values(movieData), movieId,]
			);
